    ...
    public class DEMO_LegsAnim_SpiderMover : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (moveDirectionLocal != Vector2.zero)
            {
                ...
                moveDirectionWorld = transform.rotation * new Vector3(moveDirectionLocal.x, 0, moveDirectionLocal.y);

                // moveDirectionWorld = transform.rotation * Vector3.forward;

                lastTargetAngle = FEngineering.GetAngleDeg(moveDirectionWorld.x, moveDirectionWorld.z);

                float rotationAngleDelta = 30f;
                lastTargetAngle += moveDirectionLocal.x * rotationAngleDelta;

                Debug.Log(lastTargetAngle);
                // if (Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.LeftArrow))
                // {
                //     lastTargetAngle -= rotationAngleDelta;
                // }
            ...
            if (moveDirectionLocal.y != 0)
            {


                Vector3 targetOffset = moveDirectionLocal.y > 0 ? transform.rotation * Vector3.forward : transform.rotation * -Vector3.forward;
