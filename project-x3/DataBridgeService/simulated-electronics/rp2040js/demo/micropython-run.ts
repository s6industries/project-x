...
function addMCUListeners() {
// setup listeners for MCU/circuit state
mcu.gpio[25].addListener((state: GPIOPinState, oldState: GPIOPinState) => {
  // console.log(state);
  // console.log(oldState);
  let mcuState = { "GPIO25": state }
  ws.send(JSON.stringify(mcuState));
});

mcu.gpio[1].addListener((state: GPIOPinState, oldState: GPIOPinState) => {
	// console.log(state);
	// console.log(oldState);
	let mcuState = { "GPIO1": state }
	ws.send(JSON.stringify(mcuState));
  });
}
...
if (!args.circuitpython) {
  ...
  imageName = args.image ?? 'RPI_PICO-20240105-v1.22.1.uf2';
